...
```{r}
##testing/practicing writing commands
...
The mean life expectancy is `r mean(gapminder$lifeExp)`.


We can clean this up further by wrapping this command within the `round()` function and specifying the number of digits we want reported for this numeric result. This time, use the following command inline _r round(mean(gapminder$lifeExp), digits=2)_ to rewrite the sentence below to include median, sd and sample size.

The mean life expectancy is `r round(mean(gapminder$lifeExp), digits=2)` years, the median life expectancy is `r round(median(gapminder$lifeExp), digits=2)`, the standard deviation of the life expectancy is `r round(sd(gapminder$lifeExp), digits=2)`, and the sample size is `r round(length(gapminder$lifeExp), digits =2)`.
...
###Plot GDP Per Capita by Contient
...
For this next example, we will use the built-in "country_colors" dataset defined in the `gapminder` package to create an updated data frame (copy of the `gapminder` dataset) called `gap_with_colors`. _NOTE: To see the built-in datasets in the `gapminder` package, type `data(package = "gapminder")` in the console._ After setting up the custom colors, the next line of R code defines which continents we plan to "keep" in our plot. This defines and object called `keepers` below. In this case, we are "keeping" Africa and Europe and the year 2007. So, we are subsetting the dataset for 2 countries and 1 year. Then the `plot()` command creates the scatterplot of life expectancy by GDP per capita with a dot representing each county within 2 continents (Africa and Europe) for only 2007. The size of each dot is represents a 3rd variable "pop" for population size.
...
# bubble plot, focus just on Americas and Asia in 1992
keepers <- with(gap_with_colors,
...
My homework is located at `[https://github.com/vbonise/Homework1] (https://github.com/vbonise/Homework1)`.
